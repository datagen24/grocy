...
**Two residuals, both stated rather than left to be found.** A successful login clears only *its own username's* rows — clearing the address's too was a bypass (nine guesses at `admin`, log in as yourself, repeat), also caught in this pull request's review. The cost of the narrower rule is that ten failures from one address hold that address for the rest of the window even after somebody else logs in successfully, and **behind a reverse proxy every request shares the proxy's address**, so that is an instance-wide hold rather than a per-household-member one. It self-heals in `LOGIN_THROTTLE_WINDOW_MINUTES` and the per-username limit is what carries the load in that deployment, but whether a shared address should be able to hold an instance at all is a design question this fix does not settle. |
