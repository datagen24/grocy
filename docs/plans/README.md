...
| 21 | [Frontend sink discipline](21-frontend-sink-discipline.md) | CodeQL, 2026-09-04 (23 alerts); **S29's standing guard** | — | small | draft — **step 0 landed** (`bdfa00c`, 2026-09-04): all 23 alerts closed, the two live sinks proved with a payload. What is left is why they were not caught — the `frontend-security` job three documents claim runs on every pull request does not exist, and the harness it would run cannot boot (demo mode 503s on its own `-1` marker). Also carries the summernote `.html()` decision |
