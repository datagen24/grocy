...
Victual.Components.DateTimePicker.GetInputElement = function()
{
...
Victual.Components.DateTimePicker.GetValue = function()
{
	return Victual.Components.DateTimePicker.GetInputElement().val();
}

/** Sets the input value directly and un-checks the shortcut checkbox if it no longer applies */
Victual.Components.DateTimePicker.SetValue = function(value, inputElement = Victual.Components.DateTimePicker.GetInputElement())
{
...
Victual.Components.DateTimePicker.Clear = function()
{
	Victual.Components.DateTimePicker.Init(true);

	Victual.Components.DateTimePicker.GetInputElement().val("");
...
Victual.Components.DateTimePicker.ChangeFormat = function(format)
{
	$(".datetimepicker").datetimepicker("destroy");
	Victual.Components.DateTimePicker.GetInputElement().data("format", format);
	Victual.Components.DateTimePicker.Init();

	if (format == "YYYY-MM-DD")
	{
		Victual.Components.DateTimePicker.GetInputElement().addClass("date-only-datetimepicker");
	}
	else
	{
		Victual.Components.DateTimePicker.GetInputElement().removeClass("date-only-datetimepicker");
	}
}
...
var startDate = null;
if (Victual.Components.DateTimePicker.GetInputElement().data('init-with-now') === true)
{
	startDate = moment().format(Victual.Components.DateTimePicker.GetInputElement().data('format'));
}
if (Victual.Components.DateTimePicker.GetInputElement().data('init-value').length > 0)
{
	startDate = moment(Victual.Components.DateTimePicker.GetInputElement().data('init-value')).format(Victual.Components.DateTimePicker.GetInputElement().data('format'));
}
...
if (Victual.Components.DateTimePicker.GetInputElement().data('limit-end-to-now') === true)
{
	limitDate = moment();
}
 ...
 */
Victual.Components.DateTimePicker.Init = function(reInit = false)
{
	if (reInit)
	{
		$(".datetimepicker").datetimepicker("destroy");
		...
		$(this).datetimepicker(
			{
				...
				buttons: {
					showToday: Victual.Components.DateTimePicker.GetInputElement().data('limit-end-to-now') !== true,
					showClose: true
				},
...
Victual.Components.DateTimePicker.GetInputElement().on('keyup', function(e)
{
	...
	{
		Victual.Components.DateTimePicker.SetValue(moment(new Date(), format, true).format(format), inputElement);
		nextInputElement.focus();
	}
	else if (value === 'x' || value === 'X') // Shorthand for never overdue
	{
		Victual.Components.DateTimePicker.SetValue(moment('2999-12-31 23:59:59').format(format), inputElement);
		nextInputElement.focus();
	}
	else if ((value.startsWith("+") || value.startsWith("-"))) // Shorthand for [+/-]n[d/m/y]
	{
		...
		if (lastCharacter == "d" || lastCharacter == "m" || lastCharacter == "y")
		{
			...
			if (lastCharacter == "d")
			{
				Victual.Components.DateTimePicker.SetValue(moment().add(n, "days").format(format));
				nextInputElement.focus();
			}
			else if (lastCharacter == "m")
			{
				Victual.Components.DateTimePicker.SetValue(moment().add(n, "months").format(format));
				nextInputElement.focus();
			}
			else if (lastCharacter == "y")
			{
				Victual.Components.DateTimePicker.SetValue(moment().add(n, "years").format(format));
				nextInputElement.focus();
			}
		}
	}
	else if (value.length === 4 && $.isNumeric(value) && Number.parseInt(value.substring(0, 2)) >= 1 && Number.parseInt(value.substring(0, 2)) <= 12) // Shorthand for MMDD
	{
		...
		Victual.Components.DateTimePicker.SetValue(date.format(format), inputElement);
		nextInputElement.focus();
	}
	else if (value.length === 8 && $.isNumeric(value)) // Shorthand for YYYYMMDD
	{
		Victual.Components.DateTimePicker.SetValue(value.replace(/(\d{4})(\d{2})(\d{2})/, '$1-$2-$3'), inputElement);
		nextInputElement.focus();
	}
	...
	else
	{
		...
		if (dateObj.isValid())
		{
			if (e.shiftKey)
			{
				...
				if (e.keyCode === 38) // Up
				{
					Victual.Components.DateTimePicker.SetValue(dateObj.add(-1, 'months').format(format), inputElement);
				}
				...
				{
					Victual.Components.DateTimePicker.SetValue(dateObj.add(-1, 'years').format(format), inputElement);
				}
			...
			else
			{
				...
				if (e.keyCode === 38) // Up
				{
					Victual.Components.DateTimePicker.SetValue(dateObj.add(-1, 'days').format(format), inputElement);
				}
				else if (e.keyCode === 40) // Down
				{
					Victual.Components.DateTimePicker.SetValue(dateObj.add(1, 'days').format(format), inputElement);
				...
				{
					Victual.Components.DateTimePicker.SetValue(dateObj.add(-1, 'weeks').format(format), inputElement);
				}
				else if (e.keyCode === 39) // Right
				{
					Victual.Components.DateTimePicker.SetValue(dateObj.add(1, 'weeks').format(format), inputElement);
	...
	$('#datetimepicker-timeago').attr("datetime", Victual.Components.DateTimePicker.GetValue());
	...
	value = Victual.Components.DateTimePicker.GetValue();
	dateObj = moment(value, format, true);
	var element = Victual.Components.DateTimePicker.GetInputElement()[0];
	if (!dateObj.isValid())
	{
	...
	{
		if (Victual.Components.DateTimePicker.GetInputElement().data('limit-end-to-now') === true && dateObj.isAfter(moment()))
		{
			element.setCustomValidity("error");
		}
		else if (Victual.Components.DateTimePicker.GetInputElement().data('limit-start-to-now') === true && dateObj.isBefore(moment()))
		{
			element.setCustomValidity("error");
		}
		else
		...
		var earlierThanLimit = Victual.Components.DateTimePicker.GetInputElement().data("earlier-than-limit");
		if (earlierThanLimit)
		{
...
Victual.Components.DateTimePicker.GetInputElement().on('input', function(e)
{
	$('#datetimepicker-timeago').attr("datetime", Victual.Components.DateTimePicker.GetValue());
	RefreshContextualTimeago(".datetimepicker-wrapper");
});
...
{
	Victual.Components.DateTimePicker.GetInputElement().trigger('input');
	Victual.Components.DateTimePicker.GetInputElement().trigger('change');
	Victual.Components.DateTimePicker.GetInputElement().trigger('keypress');
...
$("#datetimepicker-shortcut").on("click", function()
{
	if (this.checked)
	{
		...
		Victual.Components.DateTimePicker.SetValue(value);
		Victual.Components.DateTimePicker.GetInputElement().attr("readonly", "");
		$(Victual.Components.DateTimePicker.GetInputElement().data('next-input-selector')).focus();
	}
	else
	{
		Victual.Components.DateTimePicker.SetValue("");
		Victual.Components.DateTimePicker.GetInputElement().removeAttr("readonly");
