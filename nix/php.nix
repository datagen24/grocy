...
php85.buildEnv {
  extensions =
    ...
    [
      ...
      # pdo_sqlite is required unconditionally by PrerequisiteChecker — on every
      # request, on every deployment, including a PostgreSQL one. ADR-0008 retires
      # SQLite as a runtime engine and plan 10 makes the check driver-aware; when both
      # have landed, this line and the sqlite closure behind it come out of the serving
      # images and stay only in the migrate image, which needs it for
