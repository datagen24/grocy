...
// rollback-tests.php once per engine, bin/grocy-migrate several times — and a per-call
// change would mean remembering to add one every time a phase is added. See
// .devtools/coverage/README.md.
//
// Does nothing at all unless GROCY_COVERAGE_DIR is set, so the suite's normal runs are
...
if (getenv('GROCY_COVERAGE_DIR') === false)
{
	return;
}
...
(function ()
{
	$root = getenv('GROCY_ROOT') ?: dirname(__DIR__, 2);
	$outputDir = getenv('GROCY_COVERAGE_DIR');
